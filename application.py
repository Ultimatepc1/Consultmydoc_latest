...
# Allow less secure apps on gmail account to avoid problems
mail = Mail(app)
...
def article(id):
    ...
    result2=cur.execute("SELECT * FROM questionlike WHERE questionid=%s and username=%s",(id,session['username']))
    if result2>0:
        session.isquestionlike=True
    else:
        session.isquestionlike=False
    if isanswered=="Not Answered":
        session.isanswered=False
        return render_template("article.html",article=article)
    else:
        session.isanswered=True
        result3=cur.execute("Select * from answerlike where questionid=%s and username=%s",(id,session['username']))
        if result3>0:
            answerlikes=cur.fetchall()
            session.isanswerlike=True
            result4=cur.execute("SELECT a.ans_id, a.question_id, a.author, a.questiontitle, a.ans_body, a.create_date, a.Doctor_username, a.type, a.likes FROM answers a,answerlike ab WHERE a.ans_id=ab.answerid AND ab.questionid=%s AND ab.username=%s",(id,session['username']))
            answerliked=cur.fetchall()
            result2 = cur.execute("SELECT * FROM `answers` WHERE question_id=%s AND ans_id not in (SELECT  `answerid` FROM `answerlike` WHERE questionid=%s AND username=%s)",(id,id,session['username']))
            doctorans=cur.fetchall()
            return render_template("article.html",article=article,doctorans=doctorans,answerlikes=answerliked)
        else:
            session.isanswerlike=False
...
def delete_article(id):
    ...
    mysql.connection.commit()
    #Close connection
    cur.execute("Delete from questionlike where questionid=%s",[id])
    mysql.connection.commit()

    cur.execute("Delete from answerlike where questionid=%s",[id])
    mysql.connection.commit()
    cur.close()
...
#Like question
@app.route("/like_question/<string:id>",methods=['POST'])
@is_logged_in
def like_question(id):
    ...
    #Execute
    cur.execute("Update articles set likes=likes+1 WHERE id=%s",[id])

    #Commit to DB
    mysql.connection.commit()
    cur.execute("INSERT INTO `questionlike`(`username`, `questionid`) VALUES (%s,%s)",(session['username'],id) )
    mysql.connection.commit()
    #Close connection
    session.isquestionlike=True
    cur.close()

    flash('Question liked','success')
    return redirect(url_for('article',id=id))

#Unlike question
@app.route("/unlike_question/<string:id>",methods=['POST'])
@is_logged_in
def unlike_question(id):
    # Create cursor
    cur=mysql.connection.cursor()
    ...
    #Commit to DB
    mysql.connection.commit()
    cur.execute("Delete from questionlike where username=%s and questionid=%s",(session['username'],id) )
    mysql.connection.commit()
    #Close connection
    session.isquestionlike=False
    cur.close()

    flash('Question unliked','success')
    return redirect(url_for('article',id=id))

#Like answer
@app.route("/like_answer/<string:id>",methods=['POST'])
@is_logged_in
def like_answer(id):
    # Create cursor
    cur=mysql.connection.cursor()
    ...
    #Execute
    cur.execute("Update answers set likes=likes+1 WHERE ans_id=%s",[id])

    #Commit to DB
    mysql.connection.commit()
    result=cur.execute("Select * from answers where ans_id=%s",[id])
    ans=cur.fetchone()
    questionid=ans['question_id']
    cur.execute("INSERT INTO `answerlike`(`username`, `answerid`,`questionid`) VALUES (%s,%s,%s)",(session['username'],id,questionid) )
    mysql.connection.commit()
    #Close connection
    cur.close()

    flash('Answer liked','success')
    return redirect(url_for('article',id=questionid))

#Unlike answer
@app.route("/unlike_answer/<string:id>",methods=['POST'])
@is_logged_in
def unlike_answer(id):
    ...
    ans=cur.fetchone()
    questionid=ans['question_id']
    cur.execute("Delete from answerlike where username=%s and answerid=%s",(session['username'],id) )
    mysql.connection.commit()
    #Close connection
    cur.close()

    flash('Answer unliked','success')
    return redirect(url_for('article',id=questionid))
